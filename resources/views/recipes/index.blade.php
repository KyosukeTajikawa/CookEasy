    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('recipes.index_title') }}
            </h2>
            @auth
                <a href="{{ route('recipes.create') }}"
                   class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-md hover:bg-indigo-700">
                    {{ __('recipes.post_button') }}
                </a>
            @endauth
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <form method="GET" action="{{ route('recipes.index') }}"
                ...
                <div>
                    <label class="block text-xs text-gray-500 mb-1">{{ __('recipes.filter_cook_time') }}</label>
                ...
                <div>
                    <label class="block text-xs text-gray-500 mb-1">{{ __('recipes.filter_ingredient') }}</label>
            ...
            @if ($recipes->isEmpty())
                <p class="text-center text-gray-500 py-16">{{ __('recipes.no_recipes') }}</p>
            @else
                        ...
                        <a href="{{ route('recipes.show', $recipe) }}"
                           class="bg-white overflow-hidden shadow-sm rounded-lg hover:shadow-md transition">
                            ...
                            <div class="p-4">
                                ...
                                @if ($recipe->quiz)
                                    <span class="inline-block mt-1 px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full">🎯 Quiz</span>
                                @endif
